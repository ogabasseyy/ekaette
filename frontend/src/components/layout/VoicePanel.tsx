    ...
    <section className="panel-glass flex min-h-0 flex-col justify-between px-4 py-4 sm:px-5 sm:py-5">
      <div>
        ...
        <p className="mt-2 max-w-md text-xs leading-relaxed text-muted-foreground sm:text-sm">
          Start a live conversation, upload an image, negotiate value, and move to
          booking from one control surface.
        </p>
      </div>

      <div className="mt-4 rounded-2xl border border-border/70 bg-black/30 p-3.5 sm:mt-5 sm:p-5">
      ...
      <div className="mt-4 space-y-2">
        <p className="rounded-xl border border-border/70 bg-card/50 px-3 py-2 text-[11px] leading-5 text-muted-foreground sm:text-xs">
          Session:{' '}
          <span className="break-all text-foreground">{sessionId}</span>
        </p>
        {latestAgentStatus && (
          <StatusBadge variant="info" className="block break-words">
            Active agent: {prettyAgentName(latestAgentStatus.agent)} (
        ...
        {latestMemoryRecall && (
          <StatusBadge variant="info" className="block break-words">
            Context restored
            {latestMemoryRecall.customerName
              ? ` for ${latestMemoryRecall.customerName}`
              : ''}
