...
export function TranscriptionOverlay({ messages }: TranscriptionOverlayProps) {
  ...
  const latestMessage = messageCount > 0 ? messages[messageCount - 1] : null
  const latestMessageSignature = latestMessage
    ? `${latestMessage.role}:${latestMessage.partial ? '1' : '0'}:${latestMessage.text}`
    : ''
  ...
  }, [latestMessageSignature, messageCount])
