...
async def voice_callback(
    ...
    if is_duplicate_callback(sessionId, event_key):
        logger.info("AT voice callback deduplicated", extra={"at_session_id": sanitize_log(sessionId)})
        return Response(content=service_voice.build_end_xml(), media_type="application/xml")
...
async def outbound_call(
    ...
    try:
        ...
        logger.warning("AT outbound call failed", exc_info=True, extra={"to": sanitize_log(redact_pii(req.to))})
...
async def voice_transfer(
    ...
    try:
        ...
        logger.warning(
            ...
            exc_info=True,
            extra={"session_id": sanitize_log(req.session_id), "transfer_to": sanitize_log(redact_pii(req.transfer_to))},
        )
